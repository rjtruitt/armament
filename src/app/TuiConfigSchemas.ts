...
function registerMcpSchemas(pane: ConfigPane, mcpConfigs: Array<{ name: string; config: any }>): void {
  ...
  registerSchema(pane, 'mcp', {
    ...
    title: 'MCP Servers',
    fields: [
      { key: 'name', label: 'Server', width: 16, sortable: true, detailType: 'readonly' },
      { key: 'status', label: 'Status', width: 10, sortable: true, detailType: 'readonly' },
      ...
      { key: 'url', label: 'URL / Command', listVisible: false, detailType: 'text', description: 'Server endpoint or command to spawn' },
      { key: 'autoStart', label: 'Auto Start', listVisible: false, detailType: 'toggle', defaultValue: true },
      { key: 'timeout', label: 'Timeout (s)', listVisible: false, detailType: 'text', defaultValue: '30', description: 'Connection timeout in seconds' },
